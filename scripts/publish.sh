...
if [[ ${BRANCH_NAME} == release-* ]]; then
  ...
  if [[ ${PUBLISH_AS_DEFAULT} == "true" ]]; then
    echo "publish ${VERSION} as default at root directory"
    /usr/bin/gh-pages -d production -b gh-pages -a
  else
    echo "publish ${VERSION} to ${VERSION} directory"
    /usr/bin/gh-pages -d production -b gh-pages -e ${VERSION} -a
  fi
else
