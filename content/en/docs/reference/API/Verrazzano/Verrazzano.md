...
| Field | Type | Description | Required
| --- | --- | --- | --- |
| `type` | string | The ingress type.  Valid values are `LoadBalancer` and `NodePort`.  The default value is `LoadBalancer`. The `controller.service.externalIPs` must be specified in [NGINXInstallArgs](#nginx-install-args) if the ingress type is `NodePort`. For sample usage, see [Customize Ingress](/docs/setup/customizing/ingress/). | No |
...
| Field | Type | Description | Required
| --- | --- | --- | --- |
| `type` | string | The Istio ingress type.  Valid values are `LoadBalancer` and `NodePort`.  The default value is `LoadBalancer`. The `gateways.istio-ingressgateway.externalIPs` must be specified in [IstioInstallArgs](#istio-install-args) if the Istio ingress type is `NodePort`. For sample usage, see [Customize Ingress](/docs/setup/customizing/ingress/). | No |
