...
| `overrides` | [Overrides](#overrides) list | List of Overrides for the default `values.yaml` file for the component Helm chart. If you pass in multiple Overrides that reference the same values, Verrazzano will use the final one. You can find all possible values [here]( {{< release_source_url path=platform-operator/helm_config/charts/verrazzano-authproxy/values.yaml >}} ) and invalid values will be ignored. | No |
