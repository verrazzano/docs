...
To verify that the metrics template process was successful, access the [Prometheus console](docs/operations).
From the console, use the navigation bar to access Status/Targets.
On this page, you should see a target name with this formatting: `<workload-namespace>_<workload-name>_<workload-type>`.
Copy this job name to use it for future queries.
Verify that the State of this target is `UP`.
Next, use the navigation bar to access the Graph.
From here, use the job name you copied to construct this expression: `{job="<job_name>"}`
