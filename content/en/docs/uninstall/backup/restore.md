...
- The container on which the hook needs to be running is identified by the pod label selectors, followed by the container name.
...
After the restore operation is complete, you can see the hook logs using the `velero restore logs` command. Additionally, the hook logs are stored under the `/tmp` folder in the pod.
